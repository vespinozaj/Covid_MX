...
with(as.list(c(state,parameters)), {
  ...
  #Flow variables
    ...
    Death.Rate<-(Fatalities.infected.population+Fatalities.isolated.population)/Average.Duration.Of.Infectivity #[people/day]=[people]*[1]/[day]
  ...
  #Information delays
    ...
    dperceived.Death.Rate<-(Death.Rate-perceived.Death.Rate)/average.delay.time
    dperceived.Confirmed.Cases<-(Confirmed.Cases-perceived.Confirmed.Cases)/average.delay.time
    dperceived.deaths<-(deaths-perceived.deaths)/average.delay.time
  #State variables
    ddeaths<-Death.Rate
